...
function getBalance() {
  ...
  contractUBI.methods[encodedFunction](address).call().then(function(result) {
    console.log(result);
...
function getPoolBalance() {
  const encodedFunction = web3.eth.abi.encodeFunctionSignature('balanceOf(address)');
  contractUBI.methods[encodedFunction](poolContractAddress).call().then(function(result) {
    console.log(result);
    document.getElementById('balance-pool').textContent = toShortAmount(result);
  });
}

function transferUBI() {
  var quantity = document.getElementById('quantity-ubi').value;
  quantity = toLargeAmount(quantity);
  contractUBI.methods.transfer(poolContractAddress, quantity).send({from: allAccounts[0]}).then(function(tx) {
    console.log("Transaction: ", tx);
    document.getElementById('transfer-result').textContent = 'success';
  }).catch(function(err) {
    console.log("Failed with error: " + err);
    document.getElementById('transfer-result').textContent = 'error';
  });
}
...
function cancelStream() {
  ...
  contractUBI.methods.cancelStream(streamId).send({from: allAccounts[0]}).then(function(tx) {
    console.log("Transaction: ", tx);
    document.getElementById('cancel-stream-result').textContent = 'success';
  }).catch(function(err) {
    console.log("Failed with error: " + err);
    document.getElementById('cancel-stream-result').textContent = 'error';
  });
}
...
function getBalanceByStream() {
  ...
  contractUBI.methods[encodedFunction](streamId).call().then(function(result) {
    console.log(result);
...
function distributeUBI() {
  ...
  contractPool.methods.distributeUBIToRecipients(filteredAddresses, filteredAddresses.length).send({from: allAccounts[0]}).then(function(tx) {
    console.log("Transaction: ", tx);
    document.getElementById('distribute-addresses-result').textContent = 'success';
  }).catch(function(err) {
    console.log("Failed with error: " + err);
    document.getElementById('distribute-addresses-result').textContent = 'error';
  });
}
